...
* **Control de estado en comandos vacíos:** Para evitar que la terminal ejecute `help` cada vez que el usuario presione Enter con el input vacío, se condicionó este comportamiento a la clase `.terminal-pristine`. De esta forma, solo se lanza el comando `help` al presionar Enter si la sugerencia difuminada (placeholder) está activa y visible. En caso contrario, se imprime una línea vacía de prompt simulando una TTY normal de Linux.
* **Parametrización del enlace mailto:** Se añadieron parámetros de `subject` y `body` precargados al enlace de correo de la sección de contacto. El texto está redactado de forma natural y sin rayas (`—`), de acuerdo con la guía de identidad verbal, para facilitar a potenciales empleadores el inicio de la conversación.
